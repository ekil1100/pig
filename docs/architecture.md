...
- `tui`: terminal lifecycle, raw-mode restore guard, input decoding, editor state, layout, components, virtual frame rendering, and in-memory terminal testing helpers.
...
- M6: TUI foundation modules for input/editor/layout/render/components plus a live interactive loop with raw-mode TTY handling, cooked fallback for non-TTY input, and scripted tests that map agent events into a terminal transcript view.
- M7: settings/model registry/context file/resource metadata loading, app-level config runtime assembly, auth-backed model factory boundary, doctor resource diagnostics, and interactive `/reload`.
