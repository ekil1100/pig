...
Pig v1.0 is a Zig 0.16 local-first coding-agent implementation. M0 established the skeleton; M1 added the provider layer; M2 added a reusable agent runtime; M3 added built-in local coding tools; M4 added append-only session JSONL persistence and context tree indexing; M5 exposed print/json CLI modes; M6 starts the terminal UI foundation and scripted interactive mode while keeping TUI rendering independent from agent/provider/session internals.

## Modules

- `app`: CLI dispatch, build info, diagnostics, path output, print/json mode assembly, and interactive-mode glue.
